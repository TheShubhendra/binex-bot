...
def submit_code(update, context):
  ...
  prob_name = fetch_database(f"SELECT problem_name from problems WHERE problem_code='{prob_code}'")[0][0]
...
def deep_link(update, context):
  ...
  if len(text)==1:
    ...
    update.message.reply_text(f"Please Send the solution of Problem: {prob_name} ({prob_code})")
